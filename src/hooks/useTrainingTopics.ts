...
export interface TrainingTopic {
  ...
  plan_id: number | null;
  item_type: string; // "machine" or "software" - Ensuring this is always present
}
...
// Make requirement_id optional initially as it might be created in the same step
type NewTopicInsert = Omit<TrainingTopic, 'topic_id' | 'created_at' | 'requirement_id'> & {
    requirement_id?: number; // Make optional here
};
...
export const useTrainingTopics = (
  ...
  const fetchRequirement = useCallback(async (): Promise<number | null> => {
    ...
    try {
      ...
      const { data, error }: PostgrestSingleResponse<RequirementId> = await supabase
        .from(tableName)
        .select('id')
  ...
  const fetchTopics = useCallback(async () => {
    ...
    try {
       ...
       // Explicitly type the expected response array from Supabase
       const { data: topicsData, error: fetchError }: PostgrestResponse<TrainingTopic> = await supabase
      ...
      if (topicsData) {
        ...
        const formattedTopics: TrainingTopic[] = topicsData.map((topic) => ({
          ...
          item_type: topic.item_type ?? itemType, // Default to current itemType if DB value is missing
        }));
    ...
    try {
      ...
      if (!currentRequirementId) {
        ...
        if (fetchedReqId) {
          ...
          const { data: newReqData, error: reqError }: PostgrestResponse<RequirementId> = await supabase
            .from(tableName)
            .insert({
              [columnName]: itemId,
              plan_id: planId,
      ...
      // Prepare the new topic object for insertion
      const newTopicData: NewTopicInsert = {
        requirement_id: currentRequirementId, // Now we definitely have the requirement ID
        plan_id: planId,
        topic_text: topicText.trim(),
        item_type: itemType,
        updated_at: new Date().toISOString(), // Set updated_at on creation/update
        display_order: null, // Or calculate next display_order if needed
        // Set the correct item type ID based on the itemType
      ...
      const { data: insertedTopicData, error: insertError }: PostgrestSingleResponse<TrainingTopic> = await supabase
        .from('training_topics')
        .insert(newTopicData)
      ...
      const addedTopic: TrainingTopic = {
          ...
          item_type: insertedTopicData.item_type ?? itemType, // Default to current itemType
      };
  ...
  const updateTopic = async (topicId: number, topicText: string): Promise<boolean> => {
    ...
    try {
      ...
      const { error: updateError }: PostgrestResponse<void> = await supabase
        .from('training_topics')
        .update({
          topic_text: topicText.trim(),
  ...
  const deleteTopic = async (topicId: number): Promise<boolean> => {
    try {
      ...
      const { error: deleteError }: PostgrestResponse<void> = await supabase
        .from('training_topics')
        .delete()
        .eq('topic_id', topicId); // Match the specific topic ID
